...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(equa.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as equa.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria equa.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(equa.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
