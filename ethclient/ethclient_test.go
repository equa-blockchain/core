...
import (
	...
	"github.com/equa/go-equa/accounts/abi"
	"github.com/equa/go-equa/common"
	"github.com/equa/go-equa/consensus/beacon"
	"github.com/equa/go-equa/consensus/ethash"
...
// Verify that Client implements the equa interfaces.
var (
	_ = equa.ChainReader(&ethclient.Client{})
	_ = equa.TransactionReader(&ethclient.Client{})
	_ = equa.ChainStateReader(&ethclient.Client{})
	_ = equa.ChainSyncReader(&ethclient.Client{})
	_ = equa.ContractCaller(&ethclient.Client{})
	_ = equa.GasEstimator(&ethclient.Client{})
	_ = equa.GasPricer(&ethclient.Client{})
	_ = equa.LogFilterer(&ethclient.Client{})
	_ = equa.PendingStateReader(&ethclient.Client{})
	// _ = equa.PendingStateEventer(&ethclient.Client{})
	_ = equa.PendingContractCaller(&ethclient.Client{})
)
...
func newTestBackend(config *node.Config) (*node.Node, []*types.Block, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("can't create new equa service: %v", err)
	}
...
func testTransactionInBlock(t *testing.T, client *rpc.Client) {
	...
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != equa.NotFound {
		t.Fatal("error should be equa.NotFound")
	}
...
func ExampleRevertErrorData() {
	...
	call := equa.CallMsg{To: &contract, Gas: 30000}
