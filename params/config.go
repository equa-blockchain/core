// Copyright 2016 The go-equa Authors
...
import (
	...
	"github.com/equa/go-equa/params/forks"
)
...
var (
	...
	// EquaMainnetChainConfig contains the chain parameters to run a node on the EQUA main network.
	EquaMainnetChainConfig = &ChainConfig{
		ChainID:                 big.NewInt(3782), // Unique Chain ID for EQUA
		HomesteadBlock:          big.NewInt(0),
		DAOForkBlock:            nil,
		...
		TerminalTotalDifficulty: big.NewInt(0), // No mining transition for EQUA
		MergeNetsplitBlock:      nil,
		ShanghaiTime:            newUint64(0),
		CancunTime:              newUint64(0),
		...
		Clique:                  nil,
		Equa: &EquaConfig{
			Period:              12,    // 12 seconds per block
			Epoch:               7200,  // 24 hours with 12s blocks
			ThresholdShares:     2,     // 2/3 of validators needed
			MEVBurnPercentage:   80,    // 80% of MEV burned
			PoWDifficulty:       1000000, // Lightweight PoW for 1-2 seconds
			ValidatorReward:     2000000000000000000, // 2 EQUA per block
			SlashingPercentage:  50,    // 50% stake slashed for MEV extraction
		},
		BlobScheduleConfig: &BlobScheduleConfig{
			Cancun: DefaultCancunBlobConfig,
			Prague: DefaultPragueBlobConfig,
	...
	// EquaTestnetChainConfig contains the chain parameters to run a node on the EQUA test network.
	EquaTestnetChainConfig = &ChainConfig{
		ChainID:                 big.NewInt(37821), // Testnet Chain ID
		HomesteadBlock:          big.NewInt(0),
		DAOForkBlock:            nil,
		DAOForkSupport:          false,
		EIP150Block:             big.NewInt(0),
		...
		Clique:                  nil,
		Equa: &EquaConfig{
			Period:              6,     // 6 seconds per block for testnet
			Epoch:               1200,  // 2 hours with 6s blocks
			ThresholdShares:     2,     // 2/3 of validators needed
			MEVBurnPercentage:   80,    // 80% of MEV burned
			PoWDifficulty:       100000, // Easier PoW for testnet
			ValidatorReward:     1000000000000000000, // 1 EQUA per block
			SlashingPercentage:  25,    // 25% stake slashed for testnet
		},
		BlobScheduleConfig: &BlobScheduleConfig{
			Cancun: DefaultCancunBlobConfig,
			Prague: DefaultPragueBlobConfig,
...
var NetworkNames = map[string]string{
	...
	EquaMainnetChainConfig.ChainID.String(): "equa",
	EquaTestnetChainConfig.ChainID.String(): "equa-testnet",
}
...
type ChainConfig struct {
	...
	// EIP150 implements the Gas price changes (https://github.com/equa/EIPs/issues/150)
	...
	Equa               *EquaConfig         `json:"equa,omitempty"`
...
// EquaConfig is the consensus engine configs for EQUA hybrid PoS+PoW anti-MEV consensus.
type EquaConfig struct {
	Period              uint64 `json:"period"`              // Number of seconds between blocks to enforce
	Epoch               uint64 `json:"epoch"`               // Epoch length for validator set changes
	ThresholdShares     uint64 `json:"thresholdShares"`     // Number of validator shares needed for threshold decryption
	MEVBurnPercentage   uint64 `json:"mevBurnPercentage"`   // Percentage of detected MEV to burn (80%)
	PoWDifficulty       uint64 `json:"powDifficulty"`       // Lightweight PoW difficulty for randomness
	ValidatorReward     uint64 `json:"validatorReward"`     // Block reward for validators in wei
	SlashingPercentage  uint64 `json:"slashingPercentage"`  // Percentage of stake to slash for MEV extraction
}

// String implements the stringer interface, returning the consensus engine details.
func (c EquaConfig) String() string {
	return fmt.Sprintf("equa(period: %d, epoch: %d, threshold: %d, mev_burn: %d%%)",
		c.Period, c.Epoch, c.ThresholdShares, c.MEVBurnPercentage)
}
...
func (c *ChainConfig) Description() string {
	...
	case c.Equa != nil:
		banner += "Consensus: EQUA (hybrid PoS+PoW anti-MEV consensus)\n"
	default:
	...
	banner += fmt.Sprintf(" - Homestead:                   #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/homestead.md)\n", c.HomesteadBlock)
	if c.DAOForkBlock != nil {
		banner += fmt.Sprintf(" - DAO Fork:                    #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/dao-fork.md)\n", c.DAOForkBlock)
	}
	banner += fmt.Sprintf(" - Tangerine Whistle (EIP 150): #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/tangerine-whistle.md)\n", c.EIP150Block)
	banner += fmt.Sprintf(" - Spurious Dragon/1 (EIP 155): #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/spurious-dragon.md)\n", c.EIP155Block)
	banner += fmt.Sprintf(" - Spurious Dragon/2 (EIP 158): #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/spurious-dragon.md)\n", c.EIP155Block)
	banner += fmt.Sprintf(" - Byzantium:                   #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/byzantium.md)\n", c.ByzantiumBlock)
	banner += fmt.Sprintf(" - Constantinople:              #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/constantinople.md)\n", c.ConstantinopleBlock)
	banner += fmt.Sprintf(" - Petersburg:                  #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/petersburg.md)\n", c.PetersburgBlock)
	banner += fmt.Sprintf(" - Istanbul:                    #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/istanbul.md)\n", c.IstanbulBlock)
	if c.MuirGlacierBlock != nil {
		banner += fmt.Sprintf(" - Muir Glacier:                #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/muir-glacier.md)\n", c.MuirGlacierBlock)
	}
	banner += fmt.Sprintf(" - Berlin:                      #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/berlin.md)\n", c.BerlinBlock)
	banner += fmt.Sprintf(" - London:                      #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/london.md)\n", c.LondonBlock)
	if c.ArrowGlacierBlock != nil {
		banner += fmt.Sprintf(" - Arrow Glacier:               #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/arrow-glacier.md)\n", c.ArrowGlacierBlock)
	}
	if c.GrayGlacierBlock != nil {
		banner += fmt.Sprintf(" - Gray Glacier:                #%-8v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/gray-glacier.md)\n", c.GrayGlacierBlock)
	}
	banner += "\n"

	// Add a special section for the merge as it's non-obvious
	banner += "Merge configured:\n"
	banner += " - Hard-fork specification:    https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/paris.md\n"
	...
	if c.ShanghaiTime != nil {
		banner += fmt.Sprintf(" - Shanghai:                    @%-10v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/shanghai.md)\n", *c.ShanghaiTime)
	}
	if c.CancunTime != nil {
		banner += fmt.Sprintf(" - Cancun:                      @%-10v (https://github.com/equa/execution-specs/blob/master/network-upgrades/mainnet-upgrades/cancun.md)\n", *c.CancunTime)
	}
	if c.PragueTime != nil {
