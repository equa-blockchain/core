...
type wallet struct {
	...
	deriveChain     equa.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain equa.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
