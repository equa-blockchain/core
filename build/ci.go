...
import (
	...
	"github.com/equa/go-equa/crypto/signify"
	"github.com/equa/go-equa/internal/build"
	"github.com/equa/go-equa/internal/download"
	"github.com/equa/go-equa/internal/version"
)
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	...
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/equa/go-equa/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/equa/go-equa/internal/version.gitDate="+env.Date)
	}
...
func doCheckBadDeps() {
	baddeps := [][2]string{
		...
		{"github.com/equa/go-equa/core/rawdb", "github.com/equa/go-equa/ethdb/leveldb"},
		{"github.com/equa/go-equa/core/rawdb", "github.com/equa/go-equa/ethdb/pebbledb"},
	}
	tc := new(build.GoToolchain)
...
func doDockerBuildx(cmdline []string) {
	var (
		...
		hubImage = flag.String("hub", "equa/client-go", `Where to upload the docker image`)
	...
	//  - equa/client-go:latest                            - Pushes to the master branch, Geth only
	//  - equa/client-go:stable                            - Version tag publish on GitHub, Geth only
	//  - equa/client-go:alltools-latest                   - Pushes to the master branch, Geth & tools
	//  - equa/client-go:alltools-stable                   - Version tag publish on GitHub, Geth & tools
	//  - equa/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Geth only
	//  - equa/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Geth & tools
	//  - equa/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Geth only
	//  - equa/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Geth & tools
	var tags []string
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "equa/equa")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for _, distro := range debDistros {
			// Prepare the debian package with the go-equa sources.
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "equa"
...
type debMetadata struct {
	...
	// go-equa version being built. Note that this
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Ethereum Builds <fjl@equa.org>"
	}
	return debMetadata{
		PackageName: name,
		Env:         env,
